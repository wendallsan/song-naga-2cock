...
enum testMuxChannels { ATTACK1, DECAY1, SUSTAIN1, RELEASE1, MEOW1, MEOW2, MEOW3, MEOW4 };
...
double calcPole(int poleValue) {
  ...
  // return sqrt(0.999 * cos( double(ADC_MAX - poleValue) / 5.0)); // NOPE! RESULTS IN A FIXED SIGNAL AT ABOUT 2v.
  // return sqrt(0.999 * cos( double(ADC_MAX - poleValue) / 2000.0)); // this seems to work better.  
  return sqrt(0.999 * cos( double(ADC_MAX - poleValue) / 4000.0)); // this seems to work better.  
  // return sqrt(0.999 * cos( double(ADC_MAX - poleValue) / 8000.0)); // NOW I CAN'T GET A SNAPPY ATTACK TIME!
}
...
void updateEnvelope1() {
  if( isGate1 ){
    ...
    if (isRetrigger1) { // IF WE'RE RETRIGGERING, DO THIS STUFF!
      envelopeIsActive1 = true;
      isDecayState1 = false;
      targetValue1 = ENV_MAX;
      currentPole1 = attackPole1;    
  ...
  lastTrigger1 = isTrigger1; // WE'RE DONE! UPDATE THE lastTrigger1 VAR
}
void setup() {
  Serial.begin(9600);
